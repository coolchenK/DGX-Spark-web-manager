...
class Deployment(TimestampMixin, Base):
    ...
    benchmark_status: Mapped[str | None] = mapped_column(String(32))
    benchmark_tps: Mapped[float | None] = mapped_column(Float)
    benchmark_completion_tokens: Mapped[int | None] = mapped_column(Integer)
    benchmark_duration_seconds: Mapped[float | None] = mapped_column(Float)
    benchmark_tested_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    benchmark_error: Mapped[str | None] = mapped_column(Text)
