...
import { Alert, Button, Descriptions, Drawer, Flex, Form, Input, InputNumber, List, Popconfirm, Segmented, Select, Slider, Space, Steps, Switch, Tag, Typography, message } from 'antd'
...
interface DeployForm { name: string; model_id: string; model_path: string; api_model_name: string; route_alias?: string; runtime: 'vllm' | 'sglang'; image: string; port: number; context_length: number; memory_fraction: number; max_concurrency: number; trust_remote_code: boolean }
...
export function DeploymentsPage() {
  ...
  const compatibility = preview?.compatibility as { compatible: boolean; architectures: string[]; reasons: string[] } | undefined
    ...
    <div className="page-stack">
      ...
      <Drawer title="新建模型部署" width={620} open={drawerOpen} onClose={() => { setDrawerOpen(false); setPreview(null) }} extra={<Steps size="small" current={preview ? 1 : 0} items={[{ title: '配置' }, { title: '确认' }]} />}>
        <Form form={form} layout="vertical" initialValues={{ model_id: searchParams.get('model') ?? undefined, runtime: 'vllm', image: 'vllm/vllm-openai:v0.27.1', port: 8100, context_length: 32768, memory_fraction: .8, max_concurrency: 8, trust_remote_code: false }} onFinish={(values) => preview ? createMutation.mutate(values) : previewMutation.mutate(values)} onValuesChange={(changed) => { setPreview(null); if ('runtime' in changed) form.setFieldValue('image', changed.runtime === 'vllm' ? 'vllm/vllm-openai:v0.27.1' : 'sglang-inkling:specforge') }}>
          {!preview ? <><Form.Item name="model_id" label="模型" rules={[{ required: true }]}><Select loading={models.isLoading} options={models.data?.map((item) => ({ value: item.id, label: item.name }))} onChange={selectModel} /></Form.Item><Form.Item name="name" label="部署名称" rules={[{ required: true }]}><Input /></Form.Item><Form.Item name="model_path" label="模型路径" rules={[{ required: true }]}><Input readOnly /></Form.Item><Form.Item name="api_model_name" label="实例模型名称" rules={[{ required: true }]} tooltip="传给上游运行时的唯一模型名称"><Input /></Form.Item><Form.Item name="route_alias" label="共享网关别名（可选）" tooltip="多个部署填写相同别名时，网关会在健康实例间轮询"><Input placeholder="例如 qwen-production" /></Form.Item><Form.Item name="runtime" label="推理运行时"><Segmented block options={[{ label: 'vLLM', value: 'vllm' }, { label: 'SGLang', value: 'sglang' }]} /></Form.Item><Form.Item name="image" label="ARM64 镜像" rules={[{ required: true }]}><Select options={(runtime === 'vllm' ? ['vllm/vllm-openai:v0.27.1'] : ['sglang-inkling:specforge', 'lmsysorg/sglang:dev-cu13-inkling-dspark']).map((value) => ({ value, label: value }))} /></Form.Item><div className="form-grid"><Form.Item name="port" label="主机端口"><InputNumber min={1024} max={65535} /></Form.Item><Form.Item name="context_length" label="上下文长度"><InputNumber min={1024} step={1024} /></Form.Item><Form.Item name="max_concurrency" label="最大并发"><InputNumber min={1} max={1024} /></Form.Item><Form.Item name="trust_remote_code" label="信任远程代码" valuePropName="checked"><Switch /></Form.Item></div><Form.Item name="memory_fraction" label="统一内存比例"><Slider min={.05} max={.98} step={.01} tooltip={{ formatter: (value) => `${Math.round((value ?? 0) * 100)}%` }} /></Form.Item></> : <div className="deployment-preview"><Alert type={compatibility?.compatible ? 'success' : 'warning'} showIcon message={compatibility?.compatible ? '模型结构与所选运行时兼容' : '兼容性检查需要确认'} description={compatibility?.compatible ? `检测到 ${compatibility.architectures.join(', ') || 'Transformers'} 架构` : compatibility?.reasons.join('；')} /><Descriptions bordered size="small" column={1} items={[{ key: 'runtime', label: '运行时', children: String(preview.runtime) }, { key: 'image', label: 'ARM64 镜像', children: String(preview.image) }, { key: 'container', label: '容器', children: String(preview.container_name) }, { key: 'port', label: '主机端口', children: String(preview.port) }, { key: 'disk', label: '模型磁盘', children: formatBytes(Number(preview.estimated_disk_bytes ?? 0)) }, { key: 'memory', label: '估算统一内存', children: formatBytes(Number(preview.estimated_memory_bytes ?? 0)) }, { key: 'route', label: '网关模型名', children: String(preview.route_alias || form.getFieldValue('api_model_name')) }]} /><div><Typography.Title level={5}>执行动作</Typography.Title><List size="small" dataSource={operations} renderItem={(item) => <List.Item>{item}</List.Item>} /></div><div><Typography.Title level={5}>调用示例</Typography.Title><pre><code>{String(preview.api_example)}</code></pre></div><Alert type="info" showIcon message="失败回滚" description={String(preview.rollback)} /></div>}
