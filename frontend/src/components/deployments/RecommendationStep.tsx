...
function RecommendedField({
  ...
  return (
    <div className="recommendation-row">
      <Form.Item name={name} label={label} className="recommendation-field">
        {children}
      </Form.Item>
...
export function RecommendationStep({
  ...
  const quantizationMethods = recommendation?.runtime_capabilities.quantization_methods ?? ['auto']
  const quantizationOptions = [...new Set(['auto', ...quantizationMethods])].map((value) => ({
    value,
      ...
      children: (
        <div className="deployment-step">
          <div className="form-grid">
            <RecommendedField name={['generation_defaults', 'temperature']} path="generation_defaults.temperature" label="Temperature" recommended={generation('temperature')} editedFields={editedFields}>
              <InputNumber min={0} max={2} step={0.1} />
            </RecommendedField>
            <RecommendedField name={['generation_defaults', 'top_p']} path="generation_defaults.top_p" label="Top P" recommended={generation('top_p')} editedFields={editedFields}>
              <InputNumber min={0.01} max={1} step={0.05} />
            </RecommendedField>
            <RecommendedField name={['generation_defaults', 'top_k']} path="generation_defaults.top_k" label="Top K" recommended={generation('top_k')} editedFields={editedFields}>
              <InputNumber min={0} max={1_000_000} />
            </RecommendedField>
            <RecommendedField name={['generation_defaults', 'min_p']} path="generation_defaults.min_p" label="Min P" recommended={generation('min_p')} editedFields={editedFields}>
              <InputNumber min={0} max={1} step={0.01} />
            </RecommendedField>
            <RecommendedField name={['generation_defaults', 'repetition_penalty']} path="generation_defaults.repetition_penalty" label="重复惩罚" recommended={generation('repetition_penalty')} editedFields={editedFields}>
              <InputNumber min={0.01} max={2} step={0.05} />
            </RecommendedField>
            <RecommendedField name={['generation_defaults', 'presence_penalty']} path="generation_defaults.presence_penalty" label="Presence Penalty" recommended={generation('presence_penalty')} editedFields={editedFields}>
              <InputNumber min={-2} max={2} step={0.1} />
            </RecommendedField>
            <RecommendedField name={['generation_defaults', 'frequency_penalty']} path="generation_defaults.frequency_penalty" label="Frequency Penalty" recommended={generation('frequency_penalty')} editedFields={editedFields}>
              <InputNumber min={-2} max={2} step={0.1} />
            </RecommendedField>
            <RecommendedField name={['generation_defaults', 'max_tokens']} path="generation_defaults.max_tokens" label="默认最大生成 Token" recommended={generation('max_tokens')} editedFields={editedFields}>
  ...
  return (
    <section className="deployment-step" aria-labelledby="recommendation-heading">
      ...
      <div className="form-grid">
        <RecommendedField name="context_length" path="context_length" label="上下文长度" recommended={field('context_length')} editedFields={editedFields}>
          <InputNumber min={1024} max={1_048_576} step={1024} />
        </RecommendedField>
        <RecommendedField name="max_concurrency" path="max_concurrency" label="最大并发" recommended={field('max_concurrency')} editedFields={editedFields}>
          <InputNumber min={1} max={1024} />
        </RecommendedField>
        <RecommendedField name="max_batched_tokens" path="max_batched_tokens" label="批处理 Token 上限" recommended={field('max_batched_tokens')} editedFields={editedFields}>
      ...
      <RecommendedField name="memory_fraction" path="memory_fraction" label="统一内存比例" recommended={field('memory_fraction')} editedFields={editedFields}>
