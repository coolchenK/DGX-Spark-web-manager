...
本地模型列表不受影响。未配置上游时，请求未知模型仍返回 404。

上游解析出的模型默认全部提供，可以在同一页按「提供全部 / 仅提供所选」切换并逐个开关选择。
未提供的模型不会出现在 `/v1/models`，调用也返回 404，因此客户端即使知道模型名也无法绕过。请求显式提供的生成参数优先，部署保存的默认值仅填充未提供且运行时支持的字段。
